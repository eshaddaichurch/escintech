                        ...
                        <?php  
                          ...
                          if ($rsKelas->num_rows()>0) {
                            foreach ($rsKelas->result() as $row) {
                              $rsSr = $this->Konfirmasikelas_model->getSertifikat($rowRegistrasi->idjemaat, $rowRegistrasi->idkelas);
                              $link = '';
                              if ($rsSr->num_rows()>0) {
                                $link = site_url('registrasikelas/cetaksertifikat/'.$rsSr->row()->$rsSr->idregistrasi);
                                echo '
                                  <li><a href="'.$link.'" target="_blank"><i class="fa fa-times text-danger"></i> '.$row->namakelas.'</li></a>
                                ';
                              }else{
